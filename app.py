...
def set_korean_font():
    if platform.system() == 'Windows':
        ...
        # Streamlit 경고 대신 콘솔에만 출력 (set_page_config 전에 출력 금지)
        else:
            print("❗ 시스템에 한글 폰트가 설치되어 있지 않아 그래프 제목이 깨질 수 있습니다.")
...
ax1.plot(region_data["연도"], region_data["평균가격"], marker='o', color=color1)
ax1.tick_params(axis='y', labelcolor=color1)

ax2 = ax1.twinx()
color2 = "tab:red"
ax2.set_ylabel("기준금리 (%)", color=color2)
ax2.plot(region_data["연도"], region_data["기준금리"], marker='s', linestyle='--', color=color2)
